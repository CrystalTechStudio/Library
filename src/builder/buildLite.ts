  ...
  async function buildFolder(folder: Folder, depth: number, siteMap: Array<string>) {
    ...
    for (const [i, node] of folder.children.entries()) {
      if (node.type === 'folder') {
        ...
        siteMap.push('https://crystaltechstudio.github.io/Library/lite/chapters/' + node.htmlRelativePath);
      }
    }
  }

  const siteMap = [
    'https://crystaltechstudio.github.io/Library',
    'https://crystaltechstudio.github.io/Library/lite/entry.html',
  ];
